...
- `agnosys_cstr_starts_with(arg1, arg2)` → Does C string `str` start with C string `prefix`? Single-pass: walks `prefix` and compares, stopp...
- `agnosys_fsync(arg)` → (no behavioral docs)
- `agnosys_is_hex_char(arg)` → Is `ch` an ASCII hex digit (0-9, a-f, A-F)? Single definition shared by the hash/digest validator...
- `agnosys_is_name_char(arg)` → Is `ch` valid in a device-mapper / volume name (alphanumeric, `-`, `_`)? Single source for the dm...
- `agnosys_json_emit_cstr_or_null(arg1, arg2)` → Emit a JSON value for a possibly-null C string into a str_builder: `null` when the pointer is 0, ...
- `agnosys_read_fd_to_str(arg1, arg2)` → Read up to `cap` bytes from `fd` into a fresh heap buffer, NUL-terminate, and return it as a Str ...
- `agnosys_rename(arg1, arg2)` → (no behavioral docs)
- `agnosys_run_capture(arg1, arg2, arg3, arg4)` → Run a subprocess (cstr argv vec) and capture stdout into a caller buffer. Returns Ok(bytes_read) ...
- `agnosys_run_checked(arg1, arg2)` → Run a subprocess (cstr argv vec) and check for a zero exit code. Returns Ok(0) or Err(errmsg) on ...
