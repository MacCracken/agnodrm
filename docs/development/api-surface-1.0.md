...
>
> **Note (1.1.13):** the per-fn prose below is the 1.0 baseline (556 fns). The current shipping surface is **730 fns** at 1.1.13 — see [`api-surface-1.0.snapshot`](api-surface-1.0.snapshot) for the machine-checkable list (regenerated by `cyrius api-surface --scope=project --snapshot=PATH`). The +174 fns added across V1.1.0 → V1.1.13 are all additive (no removals, no signature changes); the 1.0 surface contract holds intact. Per-fn prose for the V1.1.x additions is tracked under D-3 in the [2026-05-09 internal review](reviews/2026-05-09-internal-review.md) — pending a `cyrius api-surface --update --prose` style generator (Phase 8 doc-tooling slot).


## Summary

- **1.0 baseline:** 556 fns / 20 modules (frozen 2026-04-17, shipped this prose).
- **Current at 1.1.13:** 730 fns / 20 modules (additive only; 0 removals, 0 signature changes vs 1.0).
- **Net delta:** +174 fns from V1.1.0 → V1.1.13 — derive accessors (V1.1.0), Serialize emissions (V1.1.12), hand-rolled `_to_json` shims (V1.1.12), security smoke tests (V1.1.13 H-2).
- Outliers resolved: 139 (all modules carry their module prefix — no prefix violations).
- Outliers open: 0.
