...
- ▶ **rename follow-through (remaining):**
  - **CI workflows** (`ci.yml`/`release.yml`/`consumer-integration.yml`) still say `agnosys` + loop the removed profiles (`core security storage trust system`) → sweep to `agnodrm` + `core`-only.
  - **`util.cyr` symbol rename** — its 7 internal helpers keep the `agnosys_*` prefix (used by udev/drm/deferred); rename → `agnodrm_*` for a clean identity (internal, non-breaking).
